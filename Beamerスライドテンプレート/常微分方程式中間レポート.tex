...
[6] 次の微分方程式は $\frac{a}{x}$ の形の特殊解をもつという.このとき,この方程式の一般解を求 めよ.
...
[9] $\left\{f_{n}(x)\right\}_{n}$ を区間 $I=[a, b]$ 上で定義された連続関数列とする. この $\left\{f_{n}(x)\right\}_{n}$ がある 関数 $f(x)$ に $I$ 上で一様収束するとき,
...
[10] $I=[a, b] \subset \mathbb{R}$ とする. $\mathbb{R}^{2}$ 内の有界閉集合 $D=I \times E$ 上で定義された関数 $f(x, y)$ は連続であり, その導関数 $\frac{\partial f}{\partial y}$ も $D$ で連続であるとする.このとき, 任意の $\left(x_{0}, c\right) \in D$ において, 初期値問題
...
をみたす解 $y \equiv y(x)$ が $x=x_{0}$ の近傍 $U$ でただ1つ存在することを示せ.\\
「常微分方程式論 1 」 前期中間テストに代えるレポート課題・解答例のプリント
...
となる. ゆえに $y^{2}+1=\exp \left\{\frac{2}{x}+C_{1}\right\}=e^{C_{1}} e^{2 / x}=C_{2} e^{2 / x},\left(C_{2}=e^{C_{1}}\right)$ となるから, $y^{2}=C_{2} e^{2 / x}-1$ と書き直して, 最終的に
...
与式 (2) において $x=0$ のケースを考えると, $y^{2}=0$ より $y=0$ (ゼロ解) が従う.ゼ 口解は trivial でほとんど意味がないから $x \neq 0$ としてよい.
...
ここで $X$ の関数 $F(X)$ を $F(X)=\frac{3-X^{2}}{2 X}$ と定義する. すなわち, $(* 1)$ より微分方程式 $(* 0)$ は同次形である. 同次形の場合は $y / x=u$ とおいて, $u$ に関する微分方程式に直す と必ず変数分離形に帰着できることが知られている. ここで
...
と変数変換して, 与えられた微分方程式 $(3)$ 或いは $(* 1)$ を書き直して, $(X, Y)$ に関す る微分方程式を導く. 実際, $X-2 Y=x-2 y+5,2 X-Y=2 x-y+4$ であること,お よび $d Y=d y$ かつ $d X=d x$ であることに注意すれば, 新しい $(X, Y)$ に関する方程式
...
\begin{equation*}
\frac{d Y}{d X}=-\frac{X-2 Y}{2 X-Y}=-\frac{1-2 \frac{Y}{X}}{2-\frac{Y}{X}}=F\left(\frac{Y}{X}\right) \tag{*5}
\end{equation*}


ここで $F(Z)=\frac{1-2 Z}{Z-2}$ である. この微分方程式 $(* 5)$ は同次形であるから, $Z=Y / X$ とお くことで変数分離形に帰着できる.
...
を得る、これを整理して $X Z^{\prime}=\left(Z^{2}-1\right) /(2-Z)$ となるから, 変数分離形である.した がって, 変形して
...
(Part I) まず (5) の斉次微分方程式: $x \frac{d z}{d x}+(1-x) z=0 \quad \cdots \cdots(* 1)$ を解く.
...
(Part II) 次に元々の非斉次方程式 (5) の一般解を求める. $(* 2)$ に対して, 定数変化法を 用いて (5) の解を
...
[5] の解答例 (ベルヌーイ (Bernoulli) 型微分方程式の解法)
...
(Part I) まず次の斉次方程式を解く. $w^{\prime}-4 x w=0$. ここで $w^{\prime}=4 x w$ は変数分離形 であるから, 変形して
...
（Part II） 次に定数変化法を用いて非斉次方程式の一般解を求める. 上記の結果 $(* 4)$ を 受けて, 非斉次方程式 $(* 3)$ の解を
...
で与えられる. あとは上述の $(* 6)$ 内の不定積分 $I=\int x^{3} e^{-2 x^{2}} d x$ を具体的に求めさ亢す ればよい.

(Part III) 上述の不定積分 $I$ を求めること.
...
(Part IV) 元々 Bernoulli 型微分方程式 (6) を考えていたので式 $z=1 / y^{2}$ を考慮して
...
[6] 次の微分方程式は $\frac{a}{x}$ の形の特殊解をもつという.このとき,この方程式の一般解を求
...
となる. ここで $y=2 / x$ が特殊解であったことを思い起こそう. 特殊解 $y=2 / x$ は元の 微分方程式 (7) $y^{\prime}+y^{2}=2 / x^{2}$ をみたすから, 上の式 $(* 4)$ で
...
という $u$ に関する新しい微分方程式が導き出される。これはベルヌーイ (Bernoulli) 型微分方程式に他ならない.したがって, 適当な変換により1階線形微分方程式に帰着で きる.
...
（Part II）（定数変化法により非斉次方程式の一般解を求めること）
...
Remark 1. 特殊解は $a=2$ と $a=-1$ の 2 通りの $a / x$ 型関数であった. 最初に求めた特殊解 $\frac{2}{x}$ の他方, $a=-1$ のケースの $-\frac{1}{x}$ の方は, 上述の解 $(* 13)$ で $c=0$ とおけば得ら れる。

Remark 2. ではもう一方の $a=2$ のケースに対応する特殊解 $\frac{2}{x}$ については, どうなって いるのか? 実は上の解法で求積に用いた $\frac{2}{x}$ の方は, 先の議論で最終的に得られた一般解 の具体的表現 $(* 13)$ において, $c \rightarrow \infty$ なる極限 (limit)に対応している. 実際,

$$
\lim _{c \rightarrow \infty} y(x)=\lim _{c \rightarrow \infty} \frac{2 c x^{3}+1}{c x^{4}-x}=\lim _{c \rightarrow \infty} \frac{2 x^{3}+\frac{1}{c}}{x^{4}-\frac{x}{c}}=\frac{2 x^{3}}{x^{4}}=\frac{2}{x}
$$

Remark 3. リッカチ (Riccati) 型方程式の求積で現れる Bernoulli 型方程式の指数は決 まっているのだから, 最初から

$$
y=\frac{2}{x}+\frac{1}{u}
$$

と変換して,$u$ に関する 1 階線形微分方程式を直接導いても解を得ることが可能である.
...
である.ここで求める非斉次方程式 (11) $y^{\prime \prime}-2 y^{\prime}+y=\sin x$ の一般解は線形方程式の一般論加ら

$$
\text { 「特殊解」 }+ \text { 「斉次方程式の一般解」 }
$$

の形で与えられることに注意しよう. ゆえに $[8]$ の非斉次方程式 (11) の解は
...
仮定の $f_{n} \rightarrow f$ の区間 $I$ 上一様収束は, $\varphi-\delta$ 論法で書き表すと

$$
\begin{aligned}
& \forall \varepsilon>0, \exists N \equiv N(\varepsilon) \in \mathbb{N}: \\
& \text { if } \quad n \geq N(\varepsilon) \Rightarrow\left|f_{n}(x)-f(x)\right|<\varepsilon, \quad(\forall x \in I=[a, b])
\end{aligned}
$$

ということである.したがって,この一様収束の定義から

$$
\begin{aligned}
& \forall \varepsilon>0, \exists N_{\varepsilon} \in \mathbb{N}: \\
& \text { if } \quad n \geq N_{\varepsilon} \Rightarrow\left|f_{n}(x)-f(x)\right|<\frac{\varepsilon}{b-a}, \quad(\forall x \in I=[a, b])
\end{aligned}
$$

としてもよい. すなわち, 上記評価 $\left|f_{n}-f\right|<\varepsilon /(b-a)$ が任意の $x \in I$ に対して成り立 つようにできることがわかる.

$n$ に対して

$$
\begin{aligned}
& \left|\int_{a}^{b} f_{n}(x) d x-\int_{a}^{b} f(x) d x\right|=\left|\int_{a}^{b}\left(f_{n}(x)-f(x)\right) d x\right| \leqslant \int_{a}^{b}\left|f_{n}(x)-f(x)\right| d x \\
& \leqslant \int_{a}^{b} \frac{\varepsilon}{b-a} d x=\frac{\varepsilon}{b-a} \int_{a}^{b} d x=\frac{\varepsilon}{b-a}[x]_{a}^{b}=\frac{\varepsilon}{b-a} \times(b-a)=\varepsilon
\end{aligned}
$$
...
仮定の一様収束 $\left\|f_{n}-f\right\|<\eta,(\forall \eta>0)$ に対して,

$$
\begin{aligned}
& \left|\int_{a}^{b} f_{n}(x) d x-\int_{a}^{b} f(x) d x\right| \leqslant \int_{a}^{b}\left|f_{n}(x)-f(x)\right| d x \leqslant \sup _{x \in I}\left|f_{n}(x)-f(x)\right| \cdot \int_{a}^{b} d x \\
& =\left\|f_{n}-f\right\|_{\infty} \cdot|I|<\eta \cdot|I|=\varepsilon
\end{aligned}
$$
...
(1) $D$ は $\mathbb{R}^{2}$ 内のコンパクト集合で, $f \equiv f(x, y)$ の $D$ 上連続性より
...
(2) 仮定より, $f \in C(D)$ は $D$ で連続な導関数 $\frac{\partial f}{\partial y}$ をもつから, $f_{x} \equiv f(x, \cdot) \in C^{1}(E)$ である.

したがって, $E$ 内の適当な区間 $\forall[\xi, \eta]$ において, 適当な点 $\exists \zeta(\xi<\zeta<\eta)$ がとれて
...
が成り立つ.これは関数 $f(x, y)$ の変数 $y$ に関する一様リプシッツ(Lipschitz) 条件に他 ならない.

(3) 以上の観点から, [10] の要件は授業で講義した「正規形の常微分方程式のコーシー問題の解の存在一意性定理」で課された条件とほぼ同様な条件を導出する十分条件となって いる. そのため, 定理の主張の証明にかんしてはほとんど同様な進行状況となるため, 同主張が成立することが示される.
